...
<div class="px-2 sm:px-0">
    <div class="border bg-dark border-gray mx-auto xs:w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 base-form rounded-sm px-4 sm:px-10 py-4 my-20">
        <form action="{{ route('password.update') }}" method="post">
            ...
            <h1 class="my-2 font-bold text-xl text-{{ $mainColor }}-400">Reset your password</h1>
            ...
            <div class="my-3">
                ...
                <label class="font-bold text-xs text-gray-300">{{ $input['label'] }}</label>
                <input
        ...
        <div class="mt-4 mb-2 flex justify-center">
            <p class="font-bold text-sm text-gray-300">Already remember your password?</p>&nbsp;
            <a href="{{ route('login') }}" class="transition font-bold text-sm text-{{ $mainColor }}-400 hover:text-{{ $mainColor }}-500">Join</a>
        </div>
    </div>
</div>
@stop