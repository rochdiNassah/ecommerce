...
<html lang="en" dir="ltr">
    ...
    <body class="bg-black">
        ...
        <div class="px-2 w-full sm:w-auto transition fixed -bottom-32 right-0 z-20" id="global-alert">
            <div>
                <div class="text-xs rounded-t-sm border border-gray bg-dark text-{{ $color }}-400 font-bold px-4 py-2">
                    {{ ucfirst(session('status')) }}
                </div>

                <div class="text-sm rounded-b-sm px-4 py-3 text-{{ $color }}-400 border border-gray bg-dark border-t-0 font-bold">
                    <p>{{ session('message') }}</p>
                </div>
