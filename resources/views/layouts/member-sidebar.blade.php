<div 
    class="fixed top-2 left-2 z-10 grid place-items-center space-y-1 bg-{{ $mainColor }}-100 hover:bg-{{ $mainColor }}-200 cursor-pointer transition w-12 px-3 py-2 rounded-sm"
    id="open-dashboard-sidebar"
>
    @for($i=0;$i<3;$i++)<div class="w-full h-1 bg-{{ $mainColor }}-600 rounded-sm"></div>@endfor
</div>

<div
    class="fixed lg:left-0 hidden z-10 w-full h-screen sm:w-80 bg-white transition border-r border-gray"
    id="dashboard-sidebar"
>
    <div
        class="transition bg-{{ $mainColor }}-100 hover:bg-{{ $mainColor }}-200 cursor-pointer transition w-12 px-3 py-2 fixed top-2 left-2 z-10 grid place-items-center space-y-1 rounded-sm"
